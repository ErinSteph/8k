...
function chanK(){
    ...
    xhr.onreadystatechange = function () {
      if (xhr.readyState == 4) {
        if(xhr.status == "200"){
          ...
          if(parseFloat(re.version) > parseFloat(version) && $.getVal('8k.optOut.' + re.version, 'false') != 'true'){            
            var con = confirm(
  ...
  k.trackUpdater = function(t){
    var updater = $('#update_secs');
    if(updater.innerHTML != 'Updating...'){
      if(t == true){
        $.event('threadUpdate', {}, d);
      }
      if(parseFloat(updater.innerHTML) > 15){
        return $.time(3000, function(){ k.trackUpdater(false); }, false);
      }else if(parseFloat(updater.innerHTML) < 3){
        return $.time(200, function(){ k.trackUpdater(false); }, false);       
      }else{
        return $.time(500, function(){ k.trackUpdater(false); }, false); 
      }
    }else{
      return $.time(100, function(){ k.trackUpdater(true); }, false); 
    }   
  }

  $.time(5000, function(){ k.trackUpdater(false); }, false);
  ...
  qr.spawn = function(){
    ...
    qr.quote = function(n){
      ...
      if($.getSelected() != ''){ $.onCursor($('#kqr-comment'), '>' + $.getSelected() + '\n'); }
      $('#kqr-comment').focus();
    }
      ...
      xhr.onreadystatechange = function () {
        if (xhr.readyState == 4) {
          ...
          if(re.hasOwnProperty('error')){                  
            ...
            $('#kqr-comment').value = '';
            setting.recent.push(re['id']);
            setting.recent = setting.recent.slice()
            qr.clearFile();
            $.htm($('#kqr-postbutton'), 'Post');
            $('#kqr-postbutton').removeEventListener("click", qr.abort, false);
            $('#kqr-postbutton').addEventListener("click", qr.send, false);
